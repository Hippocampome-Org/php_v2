...
?>
<?php
function get_synaptome_details($sub_synaptome, $sub, $table_name){
    $sub = substr($sub, 0, -2);
    ...
    $select_synaptome_query = "SELECT pre, AVG(cv_g), AVG(cv_tau_d), 
        ...
        AVG(max_tau_r), AVG(max_tau_f), AVG(max_u, min_g), 
        ...
        AVG(std_tau_f), AVG(std_u) from ".$table_name;
        //$where .= "and pre in (".$sub_synaptome.")";
        $rs = mysqli_query($conn_synaptome,$select_synaptome_query);
        $n=0;
        $result_synaptome_array = array();
        $result_synaptome_array = ["DG"=>array(), "CA3"=>array(), "CA1"=>array(), 
...
if(isset($_POST) && (count($_POST) > 0 )){
    ...
    if(strlen($sub_synaptome) > 1){
        ...
        get_synaptome_details($sub_synaptome, $sub,'tm_cond16');
    }
