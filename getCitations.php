...
for($i = 0; $i<$number_type; $i++)
{
	...
	$brief_citation = $citations -> retrieve_citation();
...
//	$rows[$i]['cell'][0] = '<span style="color:'.$neuronColor[$subregion].'"><strong>'.$neuron[$subregion].'</strong></span>';
	$rows[$i]['cell'][0] = "    ".'<a href="neuron_page.php?id='.$id.'" target="blank" title="'.$type->getName().'"><font color="'.$fontColor.'">'.$nickname.'</font></a>';
