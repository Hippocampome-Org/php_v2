...
class attachment_counts
{
	...
	public function retrive_by_id($id ,$id_neuron) 
    {
		...
		$query = "SELECT id, cell_id, original_id, name, type, protocol_tag FROM attachment_counts WHERE id = '$id' and cell_id = '$id_neuron'";
	...
	public function retrieve_by_referenceID($referenceID) 
    {
		$query = "SELECT id, cell_identifier, reference_ID, name_of_file_containing_figure, 'counts_figure' AS type FROM attachment_counts WHERE reference_ID= '$referenceID'";
		$rs = mysqli_query($GLOBALS['conn'],$query);
	...
	public function retrive_by_props($ref_id, $id_neuron, $id_neurite) 
    {
		...
		$query = "SELECT id, cell_identifier, reference_ID, name_of_file_containing_figure, 'counts_figure' AS type, null AS protocol_tag FROM $table WHERE reference_ID='$ref_id' AND cell_identifier='$id_neuron' AND neurite='$id_neurite'";
		$rs = mysqli_query($GLOBALS['conn'],$query);
		while(list($id,$cell_id, $original_id, $name, $type, $protocol_tag) = mysqli_fetch_row($rs))
		{	
			$this->setID($id);
	...
	public function retrive_by_props_nbym($ref_id, $source_id, $target_id) 
    {
		...
		//$query = "SELECT id, cell_id, original_id, name, type, protocol_tag FROM $table WHERE id = '$id' and cell_id = '$id_neuron'";
		//$query = "SELECT id, source_id, RefID, Figure, 'counts_figure' AS type, null AS protocol_tag FROM $table WHERE RefID='$ref_id' AND source_id=$source_id AND target_id=$target_id";
		$query = "SELECT id, source_id, RefID, Figure, 'counts_figure' AS type, null AS protocol_tag FROM $table WHERE RefID=$ref_id AND source_id=$source_id AND target_id=$target_id";
		//echo "<br>".$query;
		$rs = mysqli_query($GLOBALS['conn'],$query);
		while(list($id,$cell_id, $original_id, $name, $type, $protocol_tag) = mysqli_fetch_row($rs))
