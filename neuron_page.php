...
require_once('class/class.hc2nmo.php');
...
$hc2nmo = new hc2nmo($class_hc2nmo);
...
<link rel="stylesheet" href="/resources/demos/style.css" />
  ...
  $(function(){
	...
	$( "#list_hc2nmo" ).accordion({collapsible:true,active:null,heightStyle: "content",autoHeight:false});
				...
				print('
					<table width="80%" border="0" cellspacing="2" cellpadding="0">
						<tr>
							...
							<td align="left" width="80%" class="table_neuron_page2">
								<div id="list_hc2nmo" align="left">
									<em class="for_accordion">
										<font size="1.5" align="left">
											<b>Click here to view the list of mappings</b>
										</font>
									</em>
									<div id="accordion" align="left"
										<ul>');

			  for ($ind=0; $ind<$n_archives; $ind++){
			  	$archive = $hc2nmo->getArchive_array($ind); // Retrieve each NMO Archive from the indexed Archive Array

					print("
											<li>
												<a href='https://neuromorpho.org/MetaDataBrowseView.jsp?count=103&summary={%22neuron%22:{%22archive%22:[%22$archive%22]},%22ageWeightOperators%22:{},%22ageWeightOperations%22:{}}&browseBy=archive' target='_blank'>
													<font class='font13'>$archive</font>
												</a>");

					$hc2nmo->retrieve_neuron_names_by_id_and_archive($id,$archive);
					$n_neuron_names = $hc2nmo->getN_name();

		      print("
	      								<ul>");

		      $is_truncated_neuron_names = 0;
		      if ($n_neuron_names > 3){
		      	$n_neuron_names = 3;
		      	$is_truncated_neuron_names = 1;
		      }

		      $reason_for_inclusion_old = "";
				  for ($jnd=0; $jnd<$n_neuron_names; $jnd++){
				  	$neuron_name = $hc2nmo->getName_array($jnd); // Retrieve each NMO Archive corresponding to the Archive Array
				  	$reason_for_inclusion = $hc2nmo->getReason_for_inclusion_array($jnd);
						// print("
						// 							<li>
						// 								<a href='https://neuromorpho.org/neuron_info.jsp?neuron_name=$neuron_name' target='_blank'>
						// 									<font class='font13'>$neuron_name</font>
						// 								</a>
						...
						if ($jnd == 0){
							...
							if ($reason_for_inclusion == $reason_for_inclusion_old){
								print("
													</li>");								
							}else{
								print("
						...
						$reason_for_inclusion_old = $reason_for_inclusion;
					}
					if ($is_truncated_neuron_names){
						print("
													<li>
														<a href='https://neuromorpho.org/MetaDataBrowseView.jsp?count=103&summary={%22neuron%22:{%22archive%22:[%22$archive%22]},%22ageWeightOperators%22:{},%22ageWeightOperations%22:{}}&browseBy=archive' target='_blank'>
															<font class='font13'>more ...</font>
														</a>
													</li>
												</ul>");
					}else{
							...
							{	
								...
								if ($rep_value != NULL)
								{
									...
									if ($n_measurement > $max_n_measurement) {
										...
										else if ($std_sem == 'range') {
											$representative_error = 0;
											$value1 = number_format($value1, 1, ".", "");
											$value2 = number_format($value2, 1, ".", "");
										}
										else {
