...
class attachment_phases
{
	...
	public function retrive_by_props($ref_id, $id_neuron, $id_neurite) 
    {
		...
		$query = "SELECT id, cell_identifier, reference_ID, name_of_file_containing_figure, 'phases_figure' AS type, null AS protocol_tag FROM $table WHERE reference_ID='$ref_id' AND cell_identifier='$id_neuron' AND neurite='$id_neurite'";
		$rs = mysqli_query($GLOBALS['conn'],$query);
		while(list($id,$cell_id, $original_id, $name, $type, $protocol_tag) = mysqli_fetch_row($rs))
		{	
			$this->setID($id);
