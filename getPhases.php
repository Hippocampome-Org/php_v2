<?php
  ...
  require_once('class/class.phases.php');
...
$phases = new phases($class_phases);
...
for($i = 0; $i<$number_type; $i++)
{
	...
	$phasesArray = $phases->getElementsArray($id);
	...
	foreach($phasesArray as $phasesIteration)
	{
	    $min[$phasesIteration] = 10000; // setting min greater than max absolute value in the table.
	    $max[$phasesIteration] = -99999; // setting the max lesser than min absolute value in the table.
	    $mean[$phasesIteration] = -99999;
	}

	$getterString;
	$getterValue;
	$iterator = 0;
	$phasesValues = $phases->get_all_id($id);
	foreach ($phasesValues as $variable) {
		$iterator++;
		foreach($phasesArray as $phasesIteration)
		{
			$getterString = "get".$phasesIteration;
			...
			if((string)$getterValue===""){
				if($min[$phasesIteration]==10000){
					$min[$phasesIteration] = "";
				}
				if($max[$phasesIteration]==-99999){
				$max[$phasesIteration] = "";
			   }
			   if($mean[$phasesIteration]==-99999){
				$mean[$phasesIteration] = "";
			   }
				continue;
			}
			if($min[$phasesIteration]>$getterValue)
			{

				$min[$phasesIteration] = round($getterValue,4);
			}

			if($max[$phasesIteration] < $getterValue)
			{
				$max[$phasesIteration] = round($getterValue,4);	
			}

			if($variable->getPreferred() == 'Y')
			{
				$mean[$phasesIteration] = round($getterValue,4);
	...
	for($j = 0; $j<count($phasesArray); $j++)
	{
		if($mean[$phasesArray[$j]] == -99999)
		{
			$rows[$i]['cell'][$j+2] = '';
		...
		{
			$rows[$i]['cell'][$j+2] = '<span class="link_left font4"><a href = "#">'.$mean[$phasesArray[$j]].'<span class = "link_left">&#10;('.$min[$phasesArray[$j]].') To ('.$max[$phasesArray[$j]].')</span></a></span>';
