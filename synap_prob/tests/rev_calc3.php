...
<?php
$sql = "SELECT selected_volume FROM SynproVolumesSelected WHERE source_id=".$source_id." AND target_id=".$target_id." AND parcel = 'SMi';";
...
axon_values = Array();
<?php
$sql = "SELECT * FROM SynproLengthsHullVols WHERE unique_id=$source_id AND full_loc='DG:SMi:A';";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
...
<?php
$sql = "SELECT * FROM SynproLengthsHullVols WHERE unique_id=$target_id AND full_loc='DG:SMi:D';";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
...
for (let i = 0; i < parcels.length; i++) {
    //if (parcels[i] == "H") {
        for (let j = 0; j < axon_values.length; j++) {
            ...
            //document.write(axon_values[j][2]+" "+axon_values[j][4]+" "+axon_values[j][5]+" "+axon_values[j][6]+" "+axon_values[j][7]+"<br>\n");
            //calcs += parcels[i];
            //calcs += axon_values[j][2];
            //if (curr_parcel == "H") {
                //calcs += "i";
                //calcs += "\n";
            if (parcels[i] == curr_parcel) {
...
overlap_volume_stdev[1] = Math.sqrt(Math.pow(axonal_volume_stdev[1],2)+Math.pow(dendritic_volume_stdev[1],2));
...
nc_stdev_total = Math.sqrt(Math.pow(nc_stdev[1],2)) + Math.sqrt(Math.pow(nc_stdev[3],2));
...
<?php 
    echo "calcs += \"neuron ids: ($source_id,$target_id)\\n\";\n";
?>
...
calcs += "parcel 1\n";
calcs += "nps_mean[1] = (c * axonal_length_mean[1] * dendritic_length_mean[1]) / parcel_volumes[1]; \nnps_mean[1] = ("+c+" * "+axonal_length_mean[1]+" * "+dendritic_length_mean[1]+") / "+parcel_volumes[1]+"\n";
calcs += "nc_mean[1] = (1/n_parcels) + ((c * axonal_length_mean[1] * dendritic_length_mean[1]) / overlap_volume_mean[1]);\n";
calcs += "nc_mean[1] = "+(1/n_parcels)+" + "+"(("+c+" * "+axonal_length_mean[1]+" * "+dendritic_length_mean[1]+") / "+overlap_volume_mean[1]+");\n";
calcs += "nc_stdev[1] = nc_mean[1] * Math.sqrt(Math.pow((axonal_length_stdev[1]/axonal_length_mean[1]),2)+\nMath.pow((dendritic_length_stdev[1]/dendritic_length_mean[1]),2)+Math.pow((overlap_volume_stdev[1]/overlap_volume_mean[1]),2));\n";
calcs += "nc_stdev[1] = "+nc_mean[1]+" * Math.sqrt(Math.pow(("+axonal_length_stdev[1]+"/"+axonal_length_mean[1]+"),2)+Math.pow(("+dendritic_length_stdev[1]+"/\n"+dendritic_length_mean[1]+"),2)+Math.pow(("+overlap_volume_stdev[1]+"/"+overlap_volume_mean[1]+"),2));\n";
calcs += "parcel 3\n";
calcs += "nps_mean[3] = (c * axonal_length_mean[3] * dendritic_length_mean[3]) / parcel_volumes[3]; \nnps_mean[3] = ("+c+" * "+axonal_length_mean[3]+" * "+dendritic_length_mean[3]+") / "+parcel_volumes[3]+"\n";
calcs += "nc_mean[3] = (1/n_parcels) + ((c * axonal_length_mean[3] * dendritic_length_mean[3]) / overlap_volume_mean[3]);\n";
calcs += "nc_mean[3] = "+(1/n_parcels)+" + "+"(("+c+" * "+axonal_length_mean[3]+" * "+dendritic_length_mean[3]+") / "+overlap_volume_mean[3]+");\n";
